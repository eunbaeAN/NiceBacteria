...
process ASSEMBLED_FASTANI {
	...
	input: 
	tuple val(meta), path(assembly_results_fasta), path(assembled_barrnap_xls_results)

	output:
	tuple val(meta), path("${meta.id}_Ortho_ANI_analysis/${meta.id}_Ortho_ANI_analysis.out"), emit: assembled_fastani_results
