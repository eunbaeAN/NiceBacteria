process BARRNAP {
	...
	output:
	tuple val(meta), path("${meta.id}_16S_analysis/*"), emit: barrnap_results
	tuple val(meta), path("${meta.id}_16S_analysis/results_blast/${meta.id}_blast.xls"), emit: barrnap_xls_results
	tuple val(meta), path("${meta.id}_16S_analysis/${meta.id}_16S_rRNA.fasta"), emit: barrnap_rRNA_query
